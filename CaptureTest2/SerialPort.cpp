...
DWORD WINAPI CSerialPort::CommThread(LPVOID pParam)
{
	...
	{ 
		...
		*/
		...
		default:
			{
				MessageBoxA(NULL, "接收有问题!", "Error", MB_ICONERROR | MB_SYSTEMMODAL | MB_SETFOREGROUND);
				break;
			}
...
void CSerialPort::WriteToPort(LPCTSTR string)
{
	assert(m_hComm != 0);
	memset(m_szWriteBuffer, 0, sizeof(m_szWriteBuffer));
	int len = WideCharToMultiByte(CP_ACP, 0, (LPCTSTR)string, -1, NULL, 0, NULL, NULL);
	char *m_string = new char[len + 1];
	WideCharToMultiByte(CP_ACP, 0, string, -1, m_string, len, NULL, FALSE);
	strcpy(m_szWriteBuffer, m_string);
	m_nWriteSize=strlen(m_string);
	// set event for write
	SetEvent(m_hWriteEvent);
}
...
void CSerialPort::SendData(LPCTSTR lpszData, const int nLength)
{
	...
	int len = WideCharToMultiByte(CP_ACP, 0, (LPCTSTR)lpszData, -1, NULL, 0, NULL, NULL);
	char *m_lpszData = new char[len + 1];
	WideCharToMultiByte(CP_ACP, 0, lpszData, -1, m_lpszData, len, NULL, FALSE);
	strcpy(m_szWriteBuffer, m_lpszData);
					...
					if (ERROR_SUCCESS == RegQueryValueEx(hKey, (LPCTSTR)achValue, NULL, &nValueType, strDSName, &nBuffLen)){
						...
						while(++nIndex < MaxSerialPortNum){
							if (-1 == m_nComArray[nIndex]) {
								//UNICODE编码strDSName指针右移6位，若是ASCII编码，指针右移3位
